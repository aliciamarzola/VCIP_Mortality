...
class VAEModel(pl.LightningModule):
    ...
    def calculate_elbo(self, H_t, Y_targets, a_seq, X_targets=None, num_samples=10, optimize_a=False):
        ...
        for s in range(self.tau):
            ...
            # seleciona decoder pelo modo de output (Bernoulli ou Gaussiana)
            _use_bernoulli = getattr(self.config.dataset, 'output_mode', 'gaussian') == 'bernoulli'
            _decode_Y = (self.generative_model.decoding_Y_bernoulli_loss if _use_bernoulli
                         else self.generative_model.decoding_Y_loss_2)
            ...
            if s > 0 and s < self.tau - 1:
                ...
                reg_loss = _decode_Y(Z_samples, Y_current, a)
                if self.predict_X:
            ...
            if s == self.tau - 1:
                ...
                a = a_seq_hiddens_1[:, -1, :]
                reg_losses.append(_decode_Y(Z_samples.mean(dim=0), Y_current, a))
