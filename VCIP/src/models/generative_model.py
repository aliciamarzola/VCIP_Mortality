...
class GenerativeModel(nn.Module):
    ...
    def decoding_Y_bernoulli_loss(self, Z, y_binary, a):
        """Decoder Bernoulli para desfecho binário (ex: mortalidade).
        Reutiliza decoder_pa existente; a saída é tratada como logit.
        y_binary: (batch, output_dim) com valores em {0, 1}.
        """
        logit, _ = self.decode_p_a(Z, a)          # (batch, output_dim)
        prob = torch.sigmoid(logit).clamp(1e-6, 1 - 1e-6)
        return F.binary_cross_entropy(prob, y_binary.float())
